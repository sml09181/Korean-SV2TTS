...
def train(run_id: str, syn_dir: Path, models_dir: Path, save_every: int,  backup_every: int, force_restart: bool,
    ...
    weights_fpath = model_dir / "synthesizer" / f"synthesizer.pt"
        ...
        for epoch in range(1, epochs+1):
            for i, (texts, mels, embeds, idx) in enumerate(data_loader, 1):
                ...
                if backup_every != 0 and step % backup_every == 0 :
                    backup_fpath = model_dir / "synthesizer" / f"synthesizer_{k:06d}.pt"
                    model.save(backup_fpath, optimizer)

                if save_every != 0 and step % save_every == 0 :
                    # Must save latest optimizer state to ensure that resuming training
                    # doesn't produce artifacts
                    weights_fpath = model_dir / "synthesizer" / f"synthesizer_{step:06d}.pt"
                    model.save(weights_fpath, optimizer)
