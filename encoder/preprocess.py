...
def preprocess_enc003(datasets_root: Path, out_dir: Path, skip_existing=False):
    threshold = 8
    dataset_name = "enc003_spkr"
    # Initialize the preprocessing
    dataset_root, logger = _init_preprocess_dataset(dataset_name, datasets_root, out_dir)
    if not dataset_root:
        return
    ...
    # Preprocess all speakers
    speaker_dirs = list(dataset_root.glob("*"))
    print("%s: Before filtering, there are %d speakers." % (dataset_name, len(speaker_dirs)))
    print("%s: Filter out speakers under %d samples." % (dataset_name, threshold))
    speaker_dirs = [speaker for speaker in speaker_dirs if len(os.listdir(speaker))>=threshold]
    _preprocess_speaker_dirs(speaker_dirs, dataset_name, datasets_root, out_dir, skip_existing, logger)